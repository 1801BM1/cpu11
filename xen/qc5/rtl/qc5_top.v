...
//
// Top module for QMTech Cyclone-V core board based system
...
//______________________________________________________________________________
//
// Top project module - instantiates the qc5 board itself
//
module qc5
(
   input          qc5_clock_50,     // clock input 50 MHz
                                    //
   input    [1:0] qc5_button,       // push button[1:0]
   output   [0:0] qc5_led,          // LED [0:0]
                                    //
   output         qc5_uart_txd,     // UART transmitter
   input          qc5_uart_rxd,     // UART receiver
                                    //
   inout   [15:0] qc5_dram_dq,      // SDRAM data bus 16 bits
   output  [12:0] qc5_dram_addr,    // SDRAM address bus 13 bits
   output         qc5_dram_ldqm,    // SDRAM low-byte data mask
   output         qc5_dram_udqm,    // SDRAM high-byte data mask
   output         qc5_dram_we_n,    // SDRAM write enable
   output         qc5_dram_cas_n,   // SDRAM column address strobe
   output         qc5_dram_ras_n,   // SDRAM row address strobe
   output         qc5_dram_cs_n,    // SDRAM chip select
   output   [1:0] qc5_dram_ba,      // SDRAM bank address
   output         qc5_dram_clk,     // SDRAM clock
   output         qc5_dram_cke      // SDRAM clock enable
);
...
`endif(
   ...
   .pwr_rst(pwr_rst),            // power-on reset
                                 //
   .ext_reset(qc5_button[0]),    // external reset button
...
//______________________________________________________________________________
//
assign qc5_uart_txd = uart_txd;
assign uart_rxd = qc5_uart_rxd;
...
//
assign qc5_dram_dq      = 16'hzzzz;
assign qc5_dram_addr    = 13'h0000;
assign qc5_dram_ldqm    = 1'b0;
assign qc5_dram_udqm    = 1'b0;
assign qc5_dram_we_n    = 1'b1;
assign qc5_dram_cas_n   = 1'b1;
assign qc5_dram_ras_n   = 1'b1;
assign qc5_dram_cs_n    = 1'b1;
assign qc5_dram_ba[0]   = 1'b0;
assign qc5_dram_ba[1]   = 1'b0;
assign qc5_dram_clk     = sys_clk_p;
assign qc5_dram_cke     = 1'b1;

assign qc5_led[0]       = ~tog1_out;
